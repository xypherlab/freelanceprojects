...
class Main(QtWidgets.QMainWindow):
        ...
        def __init__(self):
            ...
            self.initUI()
        def initUI(self):
                global model,flag,liveflag,cap,DHT_SENSOR,DHT_PIN,recflag,stime,timeinterval
        ...
        def analyzedata(self):
                    ...
                    self.r6.setText("RGB: Mean("+str(int(cR))+","+str(int(cG))+","+str(int(cB))+")\n         Dominant("+str(int(dominant_color[0]))+","+str(int(dominant_color[1]))+","+str(int(dominant_color[2]))+")")
                    frame=segmented_image.copy()
                    image = QtGui.QImage(frame, frame.shape[1], frame.shape[0], 
                            frame.strides[0], QtGui.QImage.Format_RGB888)
                    image=image.scaledToHeight(360)
        ...
        def Loop(self):     
                global flag,cap,liveflag,DHT_SENSOR,DHT_PIN,stime,timeinterval,temperature,humidity
                ...
                if recflag==1 and etime>=timeinterval:
                    stime=time.time()
                    def get_dominant_color(image, k=4, image_processing_size = None):
                    ...
                    self.r6.setText("RGB: Mean("+str(int(cR))+","+str(int(cG))+","+str(int(cB))+")\n         Dominant("+str(int(dominant_color[2]))+","+str(int(dominant_color[1]))+","+str(int(dominant_color[0]))+")")
                    pathfile="/home/pi/Desktop/recorddata.csv"
                    now=datetime.now()
                    timedate=str(now.strftime("%d-%m-%Y %H:%M:%S"))
                    if os.path.exists(pathfile):
                        df=pd.read_csv(pathfile,names=["Datetime","Temperature","Humidity","Mean","Dominant"],skiprows=1)
                        np_df=df.values
                        df = df.append({'Datetime':timedate,'Temperature':temperature,'Humidity':humidity,'Mean':str(rgbres),'Dominant':str(dominant_color)}, ignore_index=True)
                        df.to_csv(pathfile, index= False)
                    else:
                        columns = ['Datetime','Temperature','Humidity','Mean','Dominant']
